...
{
    ui->setupUi(this);
    this->setFixedSize(900, 450); // конечный размер окна
    setWindowTitle("LR1 Task2");
...
void MainWindow::setFigures()
{
    ...
    circle->setFlags(); // Устанавливает флаги для элемента на заданные флаги. Они определяют, можно ли выбрать или изменить элемент
...
void MainWindow::reset()
{
    ...
    ui->labelPerimeter->setText("P: - мм");
    ui->labelArea->setText("S: - мм кв");
...
{
    if (isCircle)
        ui->labelPerimeter->setText("P: " + (QString::number(circle->getPerimeter()))  + " мм");
    else
        ui->labelPerimeter->setText("P: " + (QString::number(polygon->getPerimeter()))  + " мм");
}
...
{
    if (isCircle)
        ui->labelArea->setText("S: " + QString::number(circle->getArea())  + " мм кв");
    else
        ui->labelArea->setText("S: " + QString::number(polygon->getArea())  + " мм кв");
}

// горизонтальный слайдер с углом поворота
...
// горизонтальный слайдер с размером
...
void MainWindow::setGrid()
{
    ...
    penGrid->setBrush(QBrush(QColor(230,230,0))); // (107,163,212)
    penGrid->setStyle(Qt::SolidLine);
    penGrid->setCapStyle(Qt::RoundCap);
    penGrid->setWidth(1);
    for (int i = -820; i <= 820; i += 6) grid->addToGroup(scene->addLine(i, -630, i, 630, *penGrid));
    for (int i = -630; i <= 630; i += 6) grid->addToGroup(scene->addLine(-820, i, 820, i, *penGrid));
    scene->addItem(grid);
}
